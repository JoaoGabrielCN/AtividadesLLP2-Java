...
public class SimpleGame extends ApplicationAdapter {
	...
	public void create() {
		...
		thunderSound = Gdx.audio.newSound(Gdx.files.internal("assets/thunder.mp3"));
	...
	private void spawnMovel() {
		...
		Movel movel;
		raindrop.x = thunder.x = bubble.x =  MathUtils.random(0, 800 - 64);
		raindrop.y = thunder.y = 480;
		bubble.y = -64;
		raindrop.width = 64;
		raindrop.height = 64;
		thunder.width = 286 / 5;
		thunder.height = 474 / 5;

		if (rand == 0) {
			movel = new Chuva(raindrop, dropImage, dropSound);
		} else if(rand == 1){
			movel = new Raio(thunder, raioImage, thunderSound);
		}else{
			movel = new BolaDeSabao(bubble, bubbleImage, dropSound);
		}
		moveis.add(movel);
	...
	public void render() {
		...
		font.draw(batch, "Memória usada (bytes): " + (runtime.totalMemory() - runtime.freeMemory()), 350, 400);
		batch.end();
		...
		while (iter.hasNext()) {
			...
			if (movel instanceof BolaDeSabao) {
				if (movel.getmRectangle().y + 64 > Gdx.graphics.getHeight())
					iter.remove();
